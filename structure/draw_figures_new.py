...
figure_name_template_k = prefix + '%s_k_%d_dim_%d_%s'

def get_file_dir(type):
    sysstr = platform.system()
    if sysstr == "Windows":
        file_dir_window = "C:\\Users\\Leo\\Dropbox\\records\\RLRtree\\%s\\" % type
        separator = "\\"
    elif sysstr == "Darwin":
        file_dir_window = "/Users/guanli/Dropbox/records/RLRtree/%s/" % type
        separator = '/'
    return file_dir_window, separator

print(get_file_dir("window"))
class Record:
    def __init__(self):
        self.name = ''
        self.rlalgorithm = ''
    ...
    def __str__(self):
        return self.name + ' ' + self.distribution + ' ' + str(self.dataset_size) + ' ' + str(self.skewness) + ' ' \
               + str(self.dim) + ' ' + self.rlalgorithm + ' ' + self.mlalgorithm + ' ' + self.tree_type + ' ' \
               + str(self.window) + ' ' + str(self.time) + ' ' + str(self.page_access)
#"window"
def get_info(type):
    file_dir_window, separator = get_file_dir(type)
    ...
    for i in range(0, len(file_list)):
        ...
        exp_record.rlalgorithm = names[5]
        exp_record.mlalgorithm = names[6]
        exp_record.tree_type = names[7]
        if len(names) == 9 and names[8] != '':
            exp_record.insert = int(names[8])
            exp_record.window = int(names[8])
            exp_record.k = int(names[8])
        file = open(file_path)
...
record_list_window = get_info("windowML")
for record in record_list_window:
    print(record)
# get_info("pointML")
# get_info("insertML")
# get_info("knnML")
